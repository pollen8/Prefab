...
Fabrik.Prefab = new Class( {
	...
	options : {
		'plugins' : [],
		'observables':'.fabrikLabel, .fabrikElement'
	},
	...
	initialize : function(id, options) {
		...
		this.dragGroup = new Drag.Group({
			  'filter' : function(item){ return item.hasClass('activeEl'); }
			});
		...
		window.addEvent('domready', function() {
			...
			$('prefabSave').addEvent('click', this.save.bindWithEvent(this));
		}.bind(this));
	...
	makeDragElements : function() {
		console.log(this.options.plugins.element);
		this.elementDragOpts = {'droppables': $$('.fabrikForm'),
		...
		'onDrop':function(drag, drop, e){
			...
			if(drop){
				...
				drag.set('html', this.options.plugins.element[drag.retrieve('pluginref')].drop);
	...
	getObservables : function() {
		return $(this.id).getElements(this.options.observables);
	},
	...
	activate : function(el, e) {
		...
		if (typeof(this.activeEls[el.id]) == 'undefined') {
			this.activeEls[el.id] = el;
		}
		var c = el.findClassUp('fabrikForm');
	...
	makeInterface : function() {
		var plugins = [];
		this.options.plugins.element.each(function(el, x) {
		...
		var w = this.options.plugins.element.length * 80;
		$('elementPluginList').getElement('ul').setStyle('width', w + 'px');

	},

	watchToggle : function() {
		this.togglefx = new Fx.Morph($('elementContainer'), {
			duration : 800,
			transition : Fx.Transitions.Quart.easeOut
		});

		$('toogleElementPluginList').getElement('a')
				.addEvent(
						'click',
						function(e) {
							e.stop();
							var top = $('elementContainer').getStyle('top')
									.toInt() == 0 ? -90 : 0
							this.togglefx.start( {
								'top' : top
							});
...
Selector = new Class({
	...
	initialize: function(options) {
		...
		window.addEvent('domready', function(){
			...
			this.select = new Element('div', {'class':this.options.className, styles:this.options.selectorStyle}).injectInside(document.body).hide();
		}.bind(this));
		...
		window.addEvent('mouseup', function(e){
			...
			var coords = this.getArea(e); 
			if (this.active) {
				window.fireEvent(this.options.upevent, coords);
			}
			if (coords.top.x == coords.bottom.x && coords.top.y == coords.bottom.y) {
				window.fireEvent(this.options.clearevent, e);
			}
			this.active = false;
			this.start = {x:e.event.pageX, y:e.event.pageY};
			this.setSelectorArea(e);
